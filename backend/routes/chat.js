...
// 🏥 Google Places API — Nearby Hospitals & Clinics
const fetchNearbyClinics = async (lat, lng) => {
  try {
    ...
    return data.results.slice(0, 5).map(p =>
      `- ${p.name} – https://www.google.com/maps/place/?q=place_id:${p.place_id}`
    ).join("\n");
...
router.post("/chat-stream", async (req, res) => {
  try {
    ...
    const stream = await groq.chat.completions.create({
      model: "llama-3.3-70b-versatile",
      messages: [
        { role: "system", content: SYSTEM_PROMPT(user, clinics, !!req.body.location) },
        ...req.body.messages
      ],
      stream: true
    });
